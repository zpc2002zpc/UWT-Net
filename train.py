...
def parse_args():
    ...
    parser.add_argument('--wave_level', default=2, type=int,
                        help='wtconv level')
    parser.add_argument('--model_size', default='mid', type=str,
                    choices=['small', 'mid', 'large'],
                    help='Model size: small, mid, large')
...
def main():
    ...
    if config['name'] is None:
        if config['deep_supervision']:
            config['name'] = '%s_%s_wDS' % (config['dataset'], "UWTConv")
        else:
            config['name'] = '%s_%s_woDS' % (config['dataset'], "UWTConv")
    ...
    # create model
    model = UWTNet(config['input_channels'], config['num_classes'], config['wave_level'], model_size=config["model_size"])
    ...
    shutil.copy2('UWTNet.py', f'{output_dir}/{exp_name}/')
